...
const deleteComment: Interfaces.Controllers.Async = async (req, res, next) => {
  try {
    ...
    if (!comment) {
      return next(Utils.Response.error("Comment not found", 404));
    }
    if (comment.authorId !== user.id) {
      return next(Utils.Response.error("Unauthorized", 403));
    }

    await prisma.comment.deleteMany({ where: { parentId: id } });
    await prisma.comment.delete({ where: { id } });

    return res.json(Utils.Response.success("Comment and replies deleted", 200));
  } catch (err) {
